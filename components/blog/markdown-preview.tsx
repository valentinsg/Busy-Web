...
export default function MarkdownPreview({ content }: { content: string }) {
  const [MD, setMD] = React.useState<null | {
    Component: React.ComponentType<{ children?: React.ReactNode; remarkPlugins?: unknown[]; rehypePlugins?: unknown[] }>
    remark: unknown[]
    rehype: unknown[]
  }>(null)
  ...
  React.useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancelled) setMD({ Component: ReactMarkdown as unknown as React.ComponentType<{ children?: React.ReactNode; remarkPlugins?: unknown[]; rehypePlugins?: unknown[] }>, remark: [remarkGfm, remarkBreaks], rehype: [rehypeRaw] })
      } catch (e: unknown) {
