...
interface FilesItemMenuProps {
  entry: ArchiveEntry;
  className?: string;
}

export function FilesItemMenu({ entry, className }: FilesItemMenuProps) {
  const { toast } = useToast();
  const [open, setOpen] = useState(false);

  // Use correct domain
  const shareUrl = `https://busy.com.ar/files/${entry.id}`;
  const shareTitle = entry.microcopy || 'Busy Files';
  ...
  const handleDownload = useCallback(async () => {
    try {
      ...
      a.href = url;
      a.download = `busy-files-${entry.id}.jpg`;
  ...
  const handleOpenNew = useCallback(() => {
    window.open(`/files/${entry.id}`, '_blank');
    setOpen(false);
  }, [entry.id]);
