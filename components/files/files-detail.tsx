...
export function FilesDetail({ entry }: FilesDetailProps) {
  ...
  const handleShare = useCallback(async () => {
    const shareUrl = `https://busy.com.ar/files/${entry.id}`;
    const shareTitle = entry.title || 'Busy Files';
  ...
  const handleDownload = useCallback(async (imageUrl: string, entryId: string) => {
    ...
    if (!vibesMode || !selectedFilter) {
      ...
      a.href = imageUrl;
      a.download = `busy-files-${entryId}.jpg`;
      a.click();
      return;
    }
    ...
    try {
      ...
      link.download = `busy-files-${entryId}-${selectedFilter}.jpg`;
            ...
            <div className="flex items-center gap-2">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem
                    onClick={async () => {
                      await navigator.clipboard.writeText(`https://busy.com.ar/files/${entry.id}`);
                    }}
                  >
                  ...
                  <DropdownMenuItem
                    onClick={() => window.open(`/files/${entry.id}`, '_blank')}
                  >
