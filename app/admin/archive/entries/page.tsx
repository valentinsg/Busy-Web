...
function ArchiveEntriesList() {
  ...
  const loadPage = async (pageToLoad: number, append = false) => {
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(data.error || "No se pudieron cargar las entradas del Archive.");
      }
  ...
  const handleDelete = async (entryId: string) => {
    ...
    const confirmed = window.confirm('¿Seguro que querés eliminar esta entrada del Busy Archive?');
    if (!confirmed) return;
  ...
  return (
    <div className="container mx-auto px-4 py-6 font-body">
      <div className="mb-6 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-heading font-semibold tracking-tight">Busy Archive · Entradas</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Administración de piezas subidas al Busy Archive. {total > 0 && `${total} entr${total === 1 ? "ada" : "adas"} en total.`}
          </p>
        </div>
        <div className="flex gap-2">
          <Link href="/archive" target="_blank" className="hidden md:inline-flex">
      ...
      {initialLoading ? (
        <LoadingSpinner text="Cargando Busy Archive..." />
      ) : entries.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center space-y-2">
            <p className="text-lg font-semibold">Todavía no hay entradas en el Busy Archive.</p>
                    ...
                    <div className="relative aspect-[4/5] w-full overflow-hidden rounded-md bg-muted/40">
                      {entry.thumb_url ? (
                        <Image
                          ...
                          alt={entry.microcopy || "Imagen Busy Archive"}
                          fill
                  ...
                  <div className="flex-1 flex flex-col justify-between gap-3">
                    ...
                    <div className="flex items-center justify-between gap-2 text-xs text-muted-foreground">
                      ...
                      <div className="flex flex-wrap gap-2">
                        <Link href={`/admin/archive/entries/${entry.id}/edit`}>
                          <Button variant="outline" size="sm" className="font-body gap-1">
