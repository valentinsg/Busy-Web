...
import { Calendar, Camera, HelpCircle, ImagePlus, Plus, Trash2, Upload, X } from 'lucide-react';
...
function FilesUploader() {
  const [items, setItems] = useState<FilesItemForm[]>([
    { ...EMPTY_ITEM, id: 1 },
  ]);
  ...
  const handleFieldChange = (index: number, field: keyof Omit<FilesItemForm, 'id' | 'file' | 'previewUrl'>, value: string) => {
    setItems((prev) =>
      prev.map((item, i) =>
        i === index
          ? {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      for (const item of toUpload) {
        ...
        if (item.customDate) formData.append('customDate', item.customDate);
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          ...
                          onChange={(e) => handleFieldChange(index, 'customDate', e.target.value)}
                          className="h-11"
                        />
                        <p className="text-[10px] text-muted-foreground">
                          Dejá vacío para usar la fecha actual. Usá esto para cargar fotos antiguas.
                        </p>
                      </div>
                    </div>
        ...
        <div className="sticky bottom-0 -mx-3 px-3 py-3 bg-background/95 backdrop-blur-sm border-t sm:relative sm:mx-0 sm:px-0 sm:py-0 sm:bg-transparent sm:border-0">
          ...
          >
            ...
            {isSubmitting
              ...
              : `Subir a Files${items.filter((i) => i.file).length > 1 ? ` (${items.filter((i) => i.file).length})` : ''}`}
          </Button>
        </div>
      </form>
    </div>
