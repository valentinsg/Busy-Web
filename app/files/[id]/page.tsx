import { FilesDetail } from '@/components/files/files-detail';
...
import { getFilesEntry } from '@/lib/supabase/files';
import { getServiceClient } from '@/lib/supabase/server';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

interface FilesEntryPageProps {
  params: {
    id: string;
  };
}
...
export async function generateMetadata({ params }: FilesEntryPageProps): Promise<Metadata> {
  const entry = await getFilesEntry(params.id);

  if (!entry) {
    return {
      title: 'No encontrado | Busy Files',
      description: 'Esta entrada no fue encontrada.',
    };
  }

  // Use title if available, otherwise use microcopy
  const title = entry.title || entry.microcopy || 'Busy Files';
  const description = entry.microcopy || 'Un momento de Busy Files';
  const ogImage = entry.full_url;

  // Build keywords from entry data
  const keywords = ['busy files', 'busy streetwear', 'cultura urbana', 'mar del plata'];
  ...
  return {
    ...
    openGraph: {
      ...
      description,
      type: 'article',
      url: `https://busy.com.ar/files/${entry.id}`,
...
export default async function FilesEntryPage({ params }: FilesEntryPageProps) {
  const entry = await getFilesEntry(params.id);
  ...
  const jsonLd = {
    ...
    '@type': 'ImageObject',
    name: entry.title || entry.microcopy || 'Busy Files',
    description: entry.microcopy || 'Momento capturado de Busy Files',
