...
export async function GET(
  ...
  try {
    ...
    console.error('Unexpected error in GET /api/archive/[id]:', error);
...
export async function PUT(
  ...
  try {
    ...
    console.error('Unexpected error in PUT /api/archive/[id]:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    if (fetchError) {
      ...
      console.error('Error fetching archive entry for delete:', fetchError);
      return NextResponse.json({ error: 'Failed to load entry' }, { status: 500 });
    }
    ...
    if (deleteError) {
      console.error('Error deleting archive entry:', deleteError);
      return NextResponse.json({ error: 'Failed to delete entry' }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (error) {
    console.error('Unexpected error in DELETE /api/archive/[id]:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
